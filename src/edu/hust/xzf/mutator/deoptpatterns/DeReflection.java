...
public class DeReflection extends MutateTemplate {
    ...
    public void generatePatches() {
        ...
        importPos1 = suspCodeTree.getPos();
        importPos2 = codeEndPos + fixedCodeStr.length() - getSuspiciousCodeStr().length();
        offset += 2;
    ...
    public void setImportTryCatch(boolean importTryCatch) {
        boolean imported = checkTryCatch(this.getSuspiciousCodeTree());
        if (!imported) {
            insertTryCatch = importTryCatch;
        }
    }
    ...
    public String doPostProcess(String patchedJavaFile) {
        if (insertTryCatch) {
            ...
            return patchedJavaFile.substring(0, importPos1)
                    + importStr1 + patchedJavaFile.substring(importPos1, importPos2) + importStr2
