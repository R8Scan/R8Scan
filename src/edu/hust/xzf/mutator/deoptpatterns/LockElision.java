...
public class LockElision extends MutateTemplate {
    ...
    protected static ITree readClass(ITree tree) {
    ...
    public void generatePatches() {
        ...
        String select = null;
        ITree clazz = readClass(suspCodeTree);

        for (ITree child : clazz.getChildren()) {
            if (Checker.isModifier(child.getType())) {
                if (child.getLabel().equals("abstract") || child.getLabel().equals("interface"))
                    canNew = false;
            }
        }
        String clazzName = ContextReader.readClassName(clazz);
